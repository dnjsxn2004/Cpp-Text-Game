...
void ConsoleUI::PrintPlayerMeleeAttackResult(const std::string& playerName, const std::string& monsterName, int diceNumber, int damage, bool isStunSuccess) const
{
    cout << playerName << "(이) 가 " << monsterName << "에게 일반 공격을 시도했습니다." << endl;
    cout << "주사위 결과: " << diceNumber << endl;
    cout << monsterName << "에게 " << damage << "의 피해를 입혔습니다." << endl;
    if (isStunSuccess)
    {
        cout << monsterName << "이 기절 상태가 되었습니다!" << endl;
    }
}
