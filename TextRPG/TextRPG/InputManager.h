...
public:
	...
	static int InputMessageToInt(const std::string& message);
	...
	static int InputInMassegeToRange(const std::string& message, int min, int max);

	// 문자열 입력
	static std::string InputLine();

	// 문자열 출력 ex) std::string name = InputManager::InputLine("플레이어 이름을 입력하세요: ");
	static std::string InputMessageToLine(const std::string& message);
